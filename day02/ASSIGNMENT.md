...
2. In your fork, Settings → Secrets and variables → Actions:
   ...
   - secret `EC2_SSH_KEY` = the full contents of your `.pem` file, including the
     `BEGIN` and `END` lines
3. Push any small change to `day01/oracle` on `main`. Watch the Actions tab.
4. The first run creates the package `ghcr.io/<your-username>/oracle`. The deploy job logs
...
5. Screenshot the green run and the page on your instance showing your change.
